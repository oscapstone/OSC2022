...
case "$1" in
    ...
    "test6")
        ...
        for i in $(seq 0 9)
        do
            dd if=${TEMP} skip=$(($i1024)) of=${GOLDEN} iflag=skip_bytes oflag=seek_bytes seek=$(($i5120)) bs=1024 count=1 conv=notrunc 2> /dev/null
            dd if=${TEMP} skip=$(($i1024)) of=${SSD_FILE} iflag=skip_bytes oflag=seek_bytes seek=$(($i5120)) bs=1024 count=1 conv=notrunc 2> /dev/null
        done
