...
void init_thread_sched() {
    lock();
    run_queue = sc_alloc(sizeof(list_head_t));
    wait_queue = sc_alloc(sizeof(list_head_t));
    ...
    asm volatile("msr tpidr_el1, %0" :: "r"(sc_alloc(sizeof(thread_t))));
...
void kill_zombies() {
    ...
    list_for_each(curr, run_queue) {
        if (((thread_t *)curr)->iszombie) {
            ...
            free_page(((thread_t *)curr)->stack_alloced_ptr);
            free_page(((thread_t *)curr)->kernel_stack_alloced_ptr);
        }
    }
    unlock();
}
...
thread_t *thread_create(void *start) {
    ...
    r->stack_alloced_ptr = alloc_pages(0x10);
    r->kernel_stack_alloced_ptr = alloc_pages(0x10);
