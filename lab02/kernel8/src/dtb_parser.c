...
void dtb_init() {
    ...
    parse_dt(dtb_get_initrd_callback);
    uart_hex(CPIO_ADDR);
}
...
void print_dt_info(){
    ...
    uart_hex(convert_big_to_small_endian(header->magic));
...
void parse_dt(dtb_callback_t callback){
    unsigned long addr = DT_ADDR;
    unsigned long *dt_addr = (unsigned long*)addr;
    struct fdt_header* header = (struct fdt_header*)*dt_addr;
    ...
    while((unsigned long)struct_ptr < struct_end_addr){
        ...
        if(cur_val == FDT_BEGIN_NODE){
            ...
            if(cur_len != 0){   // Not Empty Property
                ...
                callback(cur_val, cur_name, cur_name_val);
                // uart_puts("Len: ");
                ...
                // uart_puts("\r\n");
                // uart_puts("Cur Value: ");
                // uart_hex(convert_big_to_small_endian(*cur_name_val));
                // uart_puts("\r\n");
                // uart_puts("******************************************\r\n");
            }
        }
...
void dtb_get_initrd_callback(unsigned int token_type, char* name, char* data) {
    if (token_type == FDT_PROP && !strcmp(name, "linux,initrd-start")) {
        CPIO_ADDR = convert_big_to_small_endian(*data);
