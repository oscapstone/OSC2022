...
void exception_handler(){
	...
	//	uart_printf("%d\n", svc);
		if(svc==8){
			dumpState();
			return;
		}else if(svc==0){//getpid
			unsigned long ret=tidGet();

			x0Set(ret);
			return;
		}else if(svc==1){//uart_read
			unsigned long ret=uart_gets((char*)x0,(int)x1,1);

			x0Set(ret);
			return;
		}else if(svc==2){//uart_write
			int ret = uart_puts((char*)x0, (int)x1);
			...
			int ret = mbox_call((unsigned char)x0, (unsigned int*)x1);
			x0Set(ret);
			return;
		}else{
			uart_printf("TODO\n");
