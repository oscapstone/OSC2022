...
struct cmd cmd_list[SYS_CMD_NUM] = {
    ...
    {.input = "test_buddy\t\t", .description = ""},
    {.input = "test_slab\t\t", .description = ""},
};
...
void system_command(char *buf)
{
    ...
    if (strcmp(buf, "help") == 0)
        ...
        reset(100);
    else if (strcmp(buf, "loadimg") == 0)
        loadimg();
    else if (strcmp(buf, "ls") == 0)
        cpio_list();
    else if (strcmp(buf, "cat") == 0)
        cpio_cat(args);
    else if (strcmp(buf, "dtb_ls") == 0)
        print_dt_info();
    else if (strcmp(buf, "dtb_cat") == 0)
        parse_dt();
    else if (strcmp(buf, "load") == 0)
        load_app(args);
    else if (strcmp(buf, "async") == 0)
        test_uart_async();
        ...
        print_slab();
    else if (strcmp(buf, "test_buddy") == 0)
        test_buddy();
    else if (strcmp(buf, "test_slab") == 0)
        test_slab();
}
