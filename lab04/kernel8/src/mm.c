...
int reserve_mem(unsigned long addr, unsigned long size)
{
  ...
  else
  {
    ...
    // add the addr and size to be reserve
...
void init_buddy(unsigned long stat_ptr)
{
  ...
  uart_puts("Reserving startup\n");
  for (int i = 0; i < reserve_count; i++)
  {
    place_buddy(mem_itr, startup.addr[i], BUDDY_FREE);
    uart_puts("reserving addr : 0x");
    uart_put_hex(startup.addr[i]);
    uart_puts("\n");
...
void *alloc_page(unsigned int size)
{
  ...
  while (list_empty(&buddy.free_list[find_ord]))
  { // if is empty
    find_ord++;
...
void *kmalloc(unsigned long size)
{
  size = pad(size, 16);
  if (size > PAGE_SIZE / 2)
  { // if size > 2048B use allocate page
