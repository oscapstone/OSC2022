...
int traverse_device_tree(void *dtb, dtb_callback cb) {
  ...
  uintptr_t dt_struct_ptr = dtb_start + get_be_int(&header->off_dt_struct);
  uintptr_t dt_strings_ptr = dtb_start + get_be_int(&header->off_dt_strings);

  dtb_end = dtb_start + get_be_int(&(header->totalsize));
...
void get_initramfs_addr(int type, const char *name, const void *data, uint32_t size)
{
    if(type==FDT_PROP&&!compareString(name,"linux,initrd-start")){
        initramfs_start=(char *)(uintptr_t)get_be_int(data);
    }
    else if (type == FDT_PROP && !compareString(name, "linux,initrd-end"))
    {
