...
void* register_slot(struct page* p) {
  ...
  p->next_page = (struct page*)((void*)VA_START + MEMORY_BASE + FRAME_SIZE * allocate_frame(0));
...
void* kmalloc(uint32_t size) {
  ...
  for (uint32_t k = 0; k < 6; k++) {
    ...
    if (sz >= size) {
      if (pages[k] == NULL) {
        pages[k] = (struct page*)((void*)VA_START + MEMORY_BASE + FRAME_SIZE * allocate_frame(0));
...
void kfree(void *ptr) {
  ...
  for (int i = 0; i < 6; i++) {
    if (pages[i] == pg) {
      ...
      deallocate_frame(kVirtualToIndex((uint64_t)pg));
    }
  }
}
