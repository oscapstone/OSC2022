...
int vfs_mount(const char* target, const char* file_name){
  if(strcmp(target, "/") == 0){
    rootfs = malloc(sizeof(mount));
    rootfs->fs = malloc(sizeof(filesystem));
    rootfs->fs->name = malloc(NAME_LEN);
    ...
    if(result == 0){
      vnode_mount->mount = malloc(sizeof(mount));
      vnode_mount->mount->fs = malloc(sizeof(filesystem));
      vnode_mount->mount->fs->name = malloc(NAME_LEN);
