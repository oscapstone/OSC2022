...
void send_kernel(int fd, const char *file)
{
    ...
    fprintf(stderr, "### sending kernel %s [%x byte]\n", file, off);
    ...
    while (pos < 2)
    {
        ...
        for(int i=0;i<len;i++) {
            fprintf(stderr, "%x ", (unsigned char)(p[pos+i]));
        }
        if (len == -1)
        {
            perror("read()");
    ...
    // if(ack == MAGIC_CMD ^ 0x2)
    //     fprintf(stderr, "ACK\n");
    // else
    //     fprintf(stderr,"NOT ACK : [%08x]\n", ack);
        ...
        while (!done || start != end)
        {
            ...
            else
            {
                ...
                if (FD_ISSET(fd, &rfds))
                {
                    ...
                    const char *p = buf2;
                    unsigned int* u32 = (unsigned int*)buf2;
                    printf("[%d] : 0x%08x\n",len ,*u32);
                    while (p < &buf2[len])
                    {
