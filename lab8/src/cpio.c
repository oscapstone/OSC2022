...
uint64_t cpio_mode(struct cpio_newc_header* p_header) {
  char *mode = p_header->c_mode;
  uint64_t raw_mode = 0;
  for (int i = 0; i < 8; i++) {
    if (mode[i] > 'A') raw_mode = raw_mode * 16 + (mode[i] - 'A' + 10);
    else raw_mode = raw_mode * 16 + (mode[i] - '0');
  }
  return raw_mode;
}
