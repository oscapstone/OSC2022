...
int kernel_main() {
  ...
  sd_init();
  struct vfs *fat32fs;
  if (fat32_create_vfs(&fat32fs, "msdos") < 0) {
    print("[K] Failed to create fat32fs\n");
  }
  if (vfs_register(fat32fs) < 0) {
    print("[K] Failed to register fat32fs\n");
  }

  if (vfs_mkdir("/boot") < 0) {
    print("[K] Failed to mkdir /boot\n");
  }
  fat32fs->vfs_op->vfs_mount(fat32fs, "/boot");  
