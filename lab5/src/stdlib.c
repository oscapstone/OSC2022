...
void mem_print_all() {
    ...
    //     uart_puts("address: ");
    //     uart_hex((unsigned long int)block);
    //     uart_puts(" size: ");
    //     uart_hex(block->size);
    //     uart_puts(" free: ");
    //     uart_hex(block->free);
...
void *malloc(size_t size) {
    ...
    int request = (size + BLOCK_SIZE) / 1024;
    if ((size + BLOCK_SIZE) % 1024 != 0)
        request++;
    printf("request: %d\n", request);
    struct block *block = (struct block *)buddy_system_alloc(request + 10);
    block->size = request * 1024 - BLOCK_SIZE;
    block->free = 1;
    block->page_number = page_number++;
