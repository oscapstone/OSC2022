...
void add_timer(unsigned int time, timer_call_back call_back, void *args) {
    ...
    int need_update = 0;
    //core_timer_disable();
    if (!timer_list) {          // timer_list is NULL
        timer_list = timer;
        need_update = 1;
    }
...
void pop_timer() {
    ...
    if (!timer_list)
        core_timer_disable();
    unsigned int time = timer_list->expire_time - read_sysreg(cntpct_el0);
    write_sysreg(cntp_tval_el0, time);
...
void test_timer() {
	...
	uart_printf("--------------------\n");
    add_timer(3 * read_sysreg(cntfrq_el0), print_timer, "This is timer 1!");
    add_timer(2 * read_sysreg(cntfrq_el0), print_timer, "This is timer 2!");
    add_timer(1 * read_sysreg(cntfrq_el0), print_timer, "This is timer 3!");
    core_timer_enable();
}
...
void normal_timer() {
	...
	unsigned long cntfrq = read_sysreg(cntfrq_el0);
	unsigned long tmp = cntpct * 10 / cntfrq;
    log_printf("[normal_timer] time elapsed: %d.%ds\n", tmp/10, tmp%10);
