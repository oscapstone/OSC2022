...
void buddy_system_free(unsigned long ptr) {
    if ((ptr - MEMORY_BASE) % PAGESIZE != 0) {
        uart_puts("buddy_system_free: ptr is not page aligned!\n");
        return;
    }
    int index = (ptr - MEMORY_BASE) / PAGESIZE;
