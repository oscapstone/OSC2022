...
#define pgd_index(addr) (((uint64_t)addr >> PGD_SHIFT) & ((9 << 1) - 1))
...
#define pud_index(addr) (((uint64_t)addr >> PUD_SHIFT) & ((9 << 1) - 1))
#define pud_offset(pud, addr) ((pudval_t*)pud + pud_index(addr))
#define pud_none(val) (val == 0)
#define pud_set(pud, val) do{*((pudval_t*)pud) = val;}while(0)

#define pmd_index(addr) (((uint64_t)addr >> PMD_SHIFT) & ((9 << 1) - 1))
#define pmd_offset(pmd, addr) ((pmdval_t*)pmd + pmd_index(addr))
#define pmd_none(val) (val == 0)
#define pmd_set(pmd, val) do{*((pmdval_t*)pmd) = val;}while(0)

#define pte_index(addr) (((uint64_t)addr >> PTE_SHIFT) & ((9 << 1) - 1))
#define pte_offset(pte, addr) ((pteval_t*)pte + pte_index(addr))
